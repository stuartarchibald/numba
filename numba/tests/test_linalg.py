...
class TestLinalgBase(TestCase):
    ...
    def specific_sample_matrix(self, size, dtype, order, rank=None, condition=None):
        ...
        rank: the rank of the matrix, an integer value, defaults to full.
        ...
        # Build a sample matrix via inversing SVD
    ...
    # the sample matrix code is a bit involved, so we'll test it!
    def test_specific_sample_matrix(self):
        ...
        for size, dtype, order in product(sizes, self.dtypes, 'FC'):
            ...
            if minmn > 1:
                rank = minmn - 1
                A = self.specific_sample_matrix(size, dtype, order, rank=rank)
                self.assertTrue(A.shape == size)
            ...
            if minmn > 1:
                condition = 10.
                A = self.specific_sample_matrix(size, dtype, order, condition=condition)
                self.assertTrue(A.shape == size)
                np.testing.assert_allclose(np.linalg.cond(A),
                                       10.,
                                       rtol=resolution,
        ...
        def check_error(args, msg, err=ValueError):
            with self.assertRaises(err) as raises:
                self.specific_sample_matrix(*args)
            self.assertIn(msg, str(raises.exception))        
...
class TestLinalgLstsq(TestLinalgBase):
   ...
   # conditioning is poor. As a result computed ranks can differ and 
   # consequently the calculated residual can differ.
   ...
   # Suggestions for improvements are welcomed!
    ...
    def test_linalg_lstsq(self):
            ...
            if use_reconstruction:
                ...
                try:
                    ...
                    for k in out_array_idx:
                        ...
                        np.testing.assert_allclose(res_expected, res_got, rtol=10.)
        ...
        # This test takes ages! So... trim combinations at random       
        np.random.seed(17)  # repeatable seed

        # gets a random dtype
        def rnd_dt():
            return self.dtypes[np.random.randint(len(self.dtypes))]
        
        orders = ['F','C'];
        # gets a random memory order flag
        def rnd_order():
            return orders[np.random.randint(len(orders))]
        ...
        for a_size in sizes:
            ...
            if not (m == 1 or n == 1):
                ...
                a_dtype = rnd_dt()
                a_order = rnd_order()
                A = self.specific_sample_matrix(a_size, a_dtype, a_order, rank=r)
                # run the test loop
                inner_test_loop_fn(A, a_dtype)               
                ...
                # specific_sample_matrix code are linspace (1, cond, [0... if rank deficient])
                a_dtype = rnd_dt()
                a_order = rnd_order()
                A = self.specific_sample_matrix(a_size, a_dtype, a_order, condition=specific_cond)
                # run the test loop
        ...
        # check 1D and 2D are accepted for B
